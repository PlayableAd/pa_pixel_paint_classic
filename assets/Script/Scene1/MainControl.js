...
cc.Class({
    ...
    start() {
        ...
        map.layers.map((colorList, indexcolorList) => colorList.objects.map((item, indexitem) => {
            //create data include color name and number block
            data[`${item.properties[0].value}`] = {};
            data[`${item.properties[0].value}`]["name"] = colorList.name;
            data[`${item.properties[0].value}`]['quantity'] = indexitem + 1;
