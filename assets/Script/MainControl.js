...
cc.Class({
    ...
    start() {
        ...
        //     console.log('main touch', event);
        // });
        map.layers.map((colorList) => colorList.objects.map((item) => {
            // console.log("🚀 ~ file: MainControl.js ~ line 5817 ~ map.layers.map ~ item", item)
            // console.log("🚀 ~ file: MainControl.js ~ line 5817 ~ map.layers.map ~ colorList", colorList)
            var node = cc.instantiate(this.blockModal);
            node.parent = this.SocksNode;
            node.setPosition(item.x, item.y);
            node.children[0].color = new cc.Color().fromHEX(this.findColor(colorList.name));
